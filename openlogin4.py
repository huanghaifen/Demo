...
class openlogin:
    def __init__(self):
        ...
        self.ip = ip_ports[0]['ip']
        self.port = ip_ports[0]['port']
    ...
    def login(self,code):
        ...
        r1= myRequests.get(url,proxies={'http':'http://%s:%s' % (self.ip,self.port)})
        r2 = myRequests.get(url,proxies={'http':'http://%s:%s' % (self.ip,self.port)})
        ...
        if jsonData['status'] == 0:
            ...
            return  False
    def openindex(self):
        html=myRequests.get(IndexUrl,proxies={'http':'http://%s:%s' % (self.ip,self.port)})
        print html.content
