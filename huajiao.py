...
def getMysqlConn():
    conn = pymysql.connect(host='fpc.mysqldb.chinacloudapi.cn', user='fpc%fpc', passwd='ewi32@3dio', db='fpc_test', charset='utf8')
    return conn
...
def getUserCount():
    ...
    cur.execute("SELECT count(FUserId) FROM qc_huajiao_user")
    ret = cur.fetchone()
    return ret[0]
...
def getLiveCount():
    ...
    #cur.execute("USE wanghong")
    #cur.execute("set names utf8mb4")
    cur.execute("SELECT count(FLiveId) FROM qc_huajiao_live")
    ret = cur.fetchone()
    return ret[0]
...
def selectUserIds(num):
    ...
    try:
        #cur.execute("USE wanghong")
        #cur.execute("set names utf8mb4")
        cur.execute("SELECT FUserId FROM qc_huajiao_user ORDER BY FScrapedTime DESC LIMIT " + str(num))
        ret = cur.fetchall()
        return ret
    except:
...
def replaceUserData(data):
    ...
    try:
        #cur.execute("USE wanghong")
        #cur.execute("set names utf8mb4")
        cur.execute("REPLACE INTO qc_huajiao_user(FUserId,FUserName, FLevel, FFollow,FFollowed,FSupported,FExperience,FAvatar,FScrapedTime) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", (int(data['FUserId']), data['FUserName'],int(data['FLevel']),int(data['FFollow']),int(data['FFollowed']), int(data['FSupported']), int(data['FExperience']), data['FAvatar'],getNowTime())
        )
        conn.commit()
...
def replaceUserLive(data):
    ...
    try:
        ...
        #cur.execute("USE wanghong")
        #cur.execute("set names utf8mb4")
        cur.execute("REPLACE INTO qc_huajiao_live(FLiveId,FUserId,FWatches,FPraises,FReposts,FReplies,FPublishTimestamp,FTitle,FImage,FLocation,FScrapedTime) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s )", (int(data['relateid']),int(data['FUserId']), int(data['watches']),int(data['praises']),int(data['reposts']),int(data['replies']),int(data['publishtimestamp']),data['title'], data['image'], data['location'],getNowTime())
        )
        conn.commit()
    except pymysql.err.InternalError as e:
        print(e)
